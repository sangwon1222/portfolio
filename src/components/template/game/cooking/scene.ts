...
export class Patty{
    ...
    setDragEvent(){
        ...
        this.mImage.on('dragstart',(_pointer:any ,gameObject: Phaser.GameObjects.Image)=>{
        ...
        this.mImage.on('dragend',(_pointer:any)=>{
            ...
            if(this.mInStoreIndex==-1 && hasEmpty && gameObject.x < 1280 && gameObject.x> 1160 && gameObject.y>290&& gameObject.y<560 &&  this.mState==1){
                // 보관함
                this.mTimer.destroy()
    ...
    fail(){
        ...
        gsap.to(this.mImage,{x,y,duration:0.25,onComplete:()=> {this.mImage.setPosition(x,y)} })
    }
