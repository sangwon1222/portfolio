...
class Customer {
    ...
    init(){
        ...
        this.mMenuList = [
            this.mScene.add.image(this.mMenu.x+60,10,menu[Math.floor(Math.random()*3)]).setDepth(2).setOrigin(0.5,0).setScale(0.8,0.8).setAlpha(0),
            this.mScene.add.image(this.mMenu.x+60,10+80,menu[Math.floor(Math.random()*3)]).setDepth(2).setOrigin(0.5,0).setScale(0.8,0.8).setAlpha(0),
            this.mScene.add.image(this.mMenu.x+60,10+160,menu[Math.floor(Math.random()*3)]).setDepth(2).setOrigin(0.5,0).setScale(0.8,0.8).setAlpha(0),
        ]
        map(this.mMenuList,(e: Phaser.GameObjects.Image)=>this.mContainer.add(e))

        const position = [1280/2-400 , 1280/2 , 1280/2 + 400]
        gsap.to(this.mContainer,{x:position[this.mIdx], duration:3,ease: 'none',onComplete:()=>{
    ...
    remove(texture:string){
        ...
        if(!this.mMenuList.length){
            ...
            gsap.to(this.mImage,{x:1400,duration: 3,onComplete:()=>{
