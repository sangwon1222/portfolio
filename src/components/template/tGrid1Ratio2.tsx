...
export default function Grid1Ratio2({ label, period, href, children }: { label: string; period: string; href: string; children: React.ReactNode }) {
  return (
    <div className="flex flex-col desktop:grid desktop:grid-cols-3 tablet:grid tablet:grid-cols-3 gap-10 border p-40 w-full">
      ...
      <div className="flex flex-col px-10 gap-20 desktop:col-span-2 tablet:col-span-2">{children}</div>
    </div>
  );
}
